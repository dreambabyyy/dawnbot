...
class Dawn:
    ...
    def log(self, message):
        print(
            ...
            f"{Fore.BLACK + Style.BRIGHT} | {Style.RESET_ALL}{message}",
            flush=True
        )
                    ...
                    async with session.get(url=url, headers=headers) as response:
                        if response.status == 400:
                            self.log(
                                ...
                                f"{Fore.BLACK + Style.BRIGHT} {self.hide_email(email)} {Style.RESET_ALL}"
    ...
    async def process_accounts(self, app_id: str, email: str, token: str, use_proxy: bool):
        ...
        while user is None:
            ...
            if not user:
                self.log(
                    f"{Fore.MAGENTA + Style.BRIGHT}[ Account{Style.RESET_ALL}"
                    f"{Fore.BLACK + Style.BRIGHT} {hide_email} {Style.RESET_ALL}"
                    f"{Fore.MAGENTA + Style.BRIGHT}-{Style.RESET_ALL}"
                    f"{Fore.RED + Style.BRIGHT} Login Failed {Style.RESET_ALL}"
                    f"{Fore.BLACK + Style.BRIGHT}With Proxy {proxy}{Style.RESET_ALL}"
                    f"{Fore.MAGENTA + Style.BRIGHT} ]{Style.RESET_ALL}"
                )
                await asyncio.sleep(1)
            ...
            self.log(
                ...
                f"{Fore.BLACK + Style.BRIGHT} {hide_email} {Style.RESET_ALL}"
                f"{Fore.MAGENTA + Style.BRIGHT}-{Style.RESET_ALL}"
                f"{Fore.BLACK + Style.BRIGHT} Earning {total_points:.0f} Points {Style.RESET_ALL}"
                f"{Fore.MAGENTA + Style.BRIGHT}]{Style.RESET_ALL}               "
            )
            await asyncio.sleep(1)
    ...
    async def main(self):
        try:
            ...
            self.log(
                ...
                f"{Fore.BLACK + Style.BRIGHT}{len(accounts)}{Style.RESET_ALL}"
            )
            self.log(f"{Fore.CYAN + Style.BRIGHT}-{Style.RESET_ALL}"*75)
